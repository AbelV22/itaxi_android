...
export function LicensesView() {
  return <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        }} className="text-xl md:text-3xl font-bold text-primary font-sans">{currentStats.median.toLocaleString('es-ES')}€</p>
          <div className={cn("flex items-center gap-1 mt-1 text-xs md:text-sm", currentStats.change < 0 ? "text-destructive" : "text-success")}>
        ...
        <div className="card-dashboard p-3 md:p-4">
          ...
          <p style={{
          fontFamily: 'Space Grotesk'
        }} className="text-xl md:text-3xl font-bold text-success font-sans">{currentStats.min.toLocaleString('es-ES')}€</p>
        ...
        <div className="card-dashboard p-3 md:p-4">
          ...
          <p style={{
          fontFamily: 'Space Grotesk'
        }} className="text-xl md:text-3xl font-bold text-destructive font-sans">{currentStats.max.toLocaleString('es-ES')}€</p>
        ...
        <div className="card-dashboard p-3 md:p-4">
          ...
          <p style={{
          fontFamily: 'Space Grotesk'
        }} className="text-xl md:text-3xl font-bold text-info font-sans">{currentStats.totalListings}</p>
        ...
        <div className="h-48 md:h-64 mb-4">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={priceHistory}>
              ...
              fontSize: 10,
              fill: 'hsl(220, 15%, 45%)'
            }} tickFormatter={value => `${(value / 1000).toFixed(0)}k€`} domain={['dataMin - 5000', 'dataMax + 5000']} width={50} />
              ...
              <Area type="monotone" dataKey="price" stroke="hsl(45, 100%, 50%)" strokeWidth={2} fill="url(#priceGradientFull)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="card-dashboard p-4 md:p-5">
        ...
        <div className="md:hidden space-y-3">
          {listings.map(listing => <div key={listing.id} className="p-3 rounded-lg bg-accent/30 border border-border">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-foreground text-sm">{listing.source}</span>
                  {listing.verified && <Badge variant="outline" className="text-xs bg-success/10 text-success border-success/30">
                      ✓
                    </Badge>}
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {listing.includesCar && <p className="text-xs text-muted-foreground">{listing.carModel} (-{listing.carValue.toLocaleString('es-ES')}€)</p>}
                </div>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {listings.map(listing => <tr key={listing.id} className="hover:bg-muted/30 transition-colors">
                  <td className="py-3">
                    <div className="flex items-center gap-2">
